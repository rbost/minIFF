...
#define TX_BUFFER_SIZE 8 /*session_id*/ + 32 /*ephemeral_public_key*/ + 8 /*enc id*/ + 16 /* tag 1*/ + 64 /* enc pos*/ + 16 /* tag 2*/  // Define the payload size here
#define RX_BUFFER_SIZE 8 + 32 + 64                                                                                                      // Define the payload size here
...
#define BUTTON_PIN GPIO_NUM_0  // GPIO21 pin connected to button
...
void setup() {
  ...
  factory_display.clear();
  factory_display.drawString(0, 0, "minIFF responder");
  ...
  // initialize the PSR push button
  pinMode(BUTTON_PIN, INPUT);

  factory_display.drawString(0, 20, "Push PSR for setup mode");
  factory_display.display();

  delay(1000);

  if (digitalRead(BUTTON_PIN) == 0) {  // the button is pushed
                                       // TODO : use the board's button
    // enter setup mode
    mode = SetupMode;
...
void startSetupMode() {
  ...
  factory_display.clear();
  factory_display.drawString(0, 0, "minIFF responder");
  factory_display.drawString(0, 10, "Setup mode");
  factory_display.drawString(0, 20, "Connect to SSID:");
  factory_display.drawString(0, 30, WiFi.softAPSSID());
  factory_display.drawString(0, 40, "http://192.168.1.1/");
  factory_display.display();
}
...
int processChallenge() {
  ...
  memcpy(challenge, rxpacket, RX_BUFFER_SIZE);
...
void refreshDisplay() {
  factory_display.clear();
  factory_display.drawString(0, 0, "minIFF responder");
  factory_display.drawString(0, 10, "Drone id: " + String(drone_id));
  factory_display.drawString(0, 20, "Last session id: " + String(last_session_id));
  factory_display.drawString(0, 30, "Rssi:" + String(Rssi));
  factory_display.display();
}
