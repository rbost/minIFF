...
#define RX_BUFFER_SIZE 8 /*session_id*/ + 32 /*ephemeral_public_key*/ + 8 /*enc id*/ + 16 /* tag 1*/ + 64 /* enc pos*/ + 16 /* tag 2*/  // Define the payload size here
#define TX_BUFFER_SIZE 8 + 32 + 64                                                                                                      // Define the payload size here
...
void setup() {
  ...
  factory_display.clear();
  factory_display.drawString(0, 0, "minIFF interrogator");
  factory_display.drawString(0, 10, "Starting...");
  factory_display.display();
  ...
  // Serial.println("Set state SEND_CHALLENGE");
    ...
    case STATE_SEND_CHALLENGE:
      {
        ...
        // Serial.println("Set state LOWPOWER");
        state = LOWPOWER;
      }
      break;
    case STATE_PROCESS_RESPONSE:
      receiveChallengeResponse(session_counter - 1, sender_ephemeral_private_key, sender_ephemeral_public_key);
      // Serial.println("Set state STATE_SEND_CHALLENGE");
      state = STATE_RX;

      // TODO : fix the timeout mechanism
      // delay(3000); // Wait before sending a new request

      break;
    case STATE_RX:
      Serial.println("into RX mode");

      // SX126xSetRx(RX_TIMEOUT_VALUE << 6);
      // Radio.Rx(RX_TIMEOUT_VALUE);
      SimulateRX(RX_TIMEOUT_VALUE);
...
// Dirty hack here, to get around a bug with the timeout on RX (see https://github.com/Heltec-Aaron-Lee/WiFi_Kit_series/issues/205)
void SimulateRX(uint32_t timeout) {
...
void OnRxDone(uint8_t* payload, uint16_t size, int16_t rssi, int8_t snr) {
  ...
  if (rxSize != RX_BUFFER_SIZE) {
    Serial.println("Invalid rx size......");
    // Serial.println("Set state RX");
    state = STATE_RX;
  } else {
    // Serial.println("Set state PROCESS_RESPONSE");
    state = STATE_PROCESS_RESPONSE;
  }
}
...
void sendChallenge(uint64_t session_id, uint8_t sender_ephemeral_private_key[32],
  ...
  Radio.Send((uint8_t*)challenge, sizeof(challenge));
}
...
void receiveChallengeResponse(uint64_t session_id, const uint8_t sender_ephemeral_private_key[32],
  ...
  memcpy(response, rxpacket, RX_BUFFER_SIZE);
...
void refreshDisplay() {
  factory_display.clear();
  factory_display.drawString(0, 0, "minIFF interrogator");
  factory_display.drawString(0, 10, "Session id: " + String(session_counter));
  factory_display.drawString(0, 20, "Rssi:" + String(Rssi));
  factory_display.display();
}
